...
class SetViewModel: ObservableObject {
    ...
    func populatePreviousValues() {
        let historicalExercises = ExerciseManager.shared.getAllPeleExercises(activity: self.exercise)
        self.previousWeight = historicalExercises.flatMap{ $0.sets }.flatMap{ $0.repWeight }.max() ?? 0
        self.previousReps = 8
    }
