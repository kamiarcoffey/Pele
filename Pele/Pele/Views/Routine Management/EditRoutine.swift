...
struct EditRoutine: View {
    ...
    @State var exercisePoolManager = ExercisePoolManager.shared
    @State var isPresentingAddExercise = false
    @State var isPresenting = true
  
    @State var selectedExercises: [Exercise] = []
    
    var didEditExercises: ([Exercise], Bool) -> ()
    ...
    var body: some View {
        VStack {
            NavigationView {
                VStack {
                    List {
                        ForEach(exercisePoolManager.getExercisePool.filter({self.routineViewModel.routine.containsExercise($0)}), id: \.self) { thisExercise in
                            ExerciseRow(exercise: thisExercise)
                        }
                    }
                    ScrollView {
                        VStack {
                            ForEach(exercisePoolManager.getExercisePool.filter({!self.routineBeingEdited.containsExercise($0)})) { thisExercise in
                ...
                .navigationBarTitle(Text(self.routineViewModel.routine.name))
                ...
                .sheet(isPresented: $isPresentingAddExercise, content: {
                    AddExercise(isPresenting: self.$isPresentingAddExercise, didCreateExercise: { exercise in
                        self.exercisePoolManager.addExercise(exercise: exercise)
                    })
                })
